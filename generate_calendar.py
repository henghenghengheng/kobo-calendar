    ...
    for component in cal.walk():
        ...
        if rrule_val:
            ...
            try:
                ...
                events.append({"summary": summary,"start": dtstart,"end": dtend,"all_day": all_day})
        else:
            events.append({"summary": summary,"start": dtstart,"end": dtend,"all_day": all_day})
    return events
...
def generate_html(events, start_of_week):
    ...
    html.append('.day-column-header{text-align:center;font-weight:bold;height:20px;line-height:20px;font-size:0.9em;}')
    html.append('.event{position:absolute;left:2px;right:2px;background:#333;color:#fff;font-size:0.85em;padding:1px;border-radius:2px;line-height:1em;}')
    html.append('.all-day{position:absolute;left:2px;right:2px;top:20px;background:#555;color:#fff;font-size:0.85em;padding:1px;border-radius:2px;}')
 ...
 if(pw==="2603"){localStorage.setItem("kobo_pw","1");} else{document.body.innerHTML="<h1>Wrong password</h1>";}
    ...
    for i in range(7):
        ...
        for e in timed_events:
            ...
            idx = 0
            while any((top < s['top']+s['height'] and top+s['height']>s['top'] and s['idx']==idx) for s in slots):
                idx += 1
            slots.append({'top':top,'height':height,'idx':idx})
            total_slots = max([s['idx'] for s in slots if s['top']<top+height and s['top']+s['height']>top]+[0])+1
            left_pct = idx * (100/total_slots)
            width_pct = 100 / total_slots
            html.append(f'<div class="event" style="top:{top}px;height:{height}px;left:{left_pct}%;width:{width_pct}%;">'
                        f'{e["summary"]}<br><span class="time">{e["start"].strftime("%H:%M")} - {e["end"].strftime("%H:%M")}</span></div>')
