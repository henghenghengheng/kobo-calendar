    ...
    for component in cal.walk():
        ...
        if rrule_val:
            ...
            try:
                ...
                occurrences = list(rrule_obj.between(start_range, end_range, inc=True)) if start_range and end_range else list(rrule_obj)
...
def generate_html(events, start_of_week):
    ...
    .hour-labels { width:40px; display:flex; flex-direction:column; margin-top:40px; }
    .hour-labels div { height:60px; font-size:0.7em; text-align:right; padding-right:2px; border-bottom:1px solid #eee; }
    .week { display:flex; flex:1; }
    .day-column { flex:1; border-left:1px solid #ccc; position:relative; margin-left:2px; }
    .day-column-header { text-align:center; background:#eee; font-weight:bold; border-bottom:1px solid #ccc; height:20px; line-height:20px; }
    ...
    .all-day { position:absolute; left:2px; right:2px; top:20px; background:#555; color:#fff; font-size:0.7em; padding:1px; border-radius:2px; }
    """)
    ...
    html.append('</div>')  # end hour-labels
    ...
    for i in range(7):
        ...
        for e in events:
            if not e['all_day'] and e['start'].date() == day_date.date():
                hour_offset = e['start'].hour + e['start'].minute / 60
                top = (hour_offset - 8) * 60 + 20  # 8:00 → top 0
                duration_hours = (e['end'] - e['start']).total_seconds() / 3600
                height = max(20, duration_hours * 60)
...
def main():
    ...
    events = parse_events(cal, now - datetime.timedelta(days=7), now + datetime.timedelta(days=7)) if cal else []
    week_events, start_of_week = filter_week(events, now)

    # Generate HTML
    html = generate_html(week_events, start_of_week)

    # Write to calendar.html (overwrite)
    try:
        fpath = "calendar.html"
